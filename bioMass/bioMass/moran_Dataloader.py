import pandas as pd
import rasterio
import bioMass.transforms as tf
import numpy as np
import os
...
class MoranSentinelDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        if self.dir_target:
            ...
            target_tile = target_tile.clamp(target_tile.min(), 583.80999756)
        else:
            target_tile = torch.full([1, 256, 256], torch.nan, dtype=torch.float32, requires_grad=False, device=self.device)

        sample = {'image_s1': s1_tile_scaled, 'image_s2': s2_tile_scaled, 'label': target_tile} # 'image' and 'label' are used by torchgeo
